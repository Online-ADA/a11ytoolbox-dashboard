<template>
	<Modal :valign="'top'" style="z-index:999" :size="'creation'" :open="open">
		...
		<div class="flex justify-evenly items-start" v-if="myTeam == 1 || myTeam == 2">
			...
			@click.prevent="EventBus.openModal( ()=>{ EventBus.$emit('deployToolModal', false); EventBus.$emit('deployWCAGAuditModal', true); })">
			...
			<button 
			class="font-bold mr-2 text-center text-pallette-button hover:text-pallette-button-hover" 
			@click.prevent="EventBus.openModal( ()=>{ EventBus.$emit('deployToolModal', false); EventBus.$emit('deployMediaAuditModal', true); })">
