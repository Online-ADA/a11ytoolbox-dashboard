<template>
	<div ref="rootModal" :id="`a${id}`" :class="containerClasses" class="inset-0 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
		<div :class="alignClass" class="flex justify-center min-h-screen pt-4 px-4 pb-20 text-center p-0">
			...
			<div :class="{'max-w-lg': size == 'compact', 'max-w-4xl': size == 'wide', 'max-w-full': size == 'full', 'creation-max-w': size == 'creation'}" class="modal-main relative inline-block bg-white text-left overflow-hidden shadow-xl transform align-middle p-4 w-full xs:p-0">
