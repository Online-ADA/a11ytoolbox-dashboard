...
export default {
	...
	methods: {
		...
		createAudit(){
			...
			this.$store.dispatch("audits/createAudit", {audit: this.audit, router: this.$router, vm: this})
		},
	},
	created() {
		...
		if( this.$store.getters["auth/isManager"] ){
			this.$store.dispatch("audits/getAuditors", {router: this.$router, account_id: this.$store.state.auth.account, vm: this})
		}
	},
	mounted() {
