...
export default {
  ...
  state: {
    ...
    getTimeLeft() {
      ...
      const seconds_remaining = Number((this.token_expire - current_date) / 1000);
  ...
  actions: {
    login({ state }) {
      const auth_redirect = encodeURIComponent('/'); // To change to current route?
    ...
    setToken({ state, dispatch }, payload) {
      ...
      const token_expire = payload.token_expire * 1000;
